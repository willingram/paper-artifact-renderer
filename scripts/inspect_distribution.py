"""Validate PAR wheel and source-distribution contents without publishing them."""
...
EXPECTED_PROJECT_URLS = {
    "Homepage": "https://github.com/willingram/paper-artifact-renderer",
    "Repository": "https://github.com/willingram/paper-artifact-renderer",
    "Issues": "https://github.com/willingram/paper-artifact-renderer/issues",
    "Changelog": "https://github.com/willingram/paper-artifact-renderer/blob/main/CHANGELOG.md",
}
...
FORBIDDEN_SUFFIXES = (".key", ".p12", ".pem", ".pfx", ".pyc", ".pyo", ".truth.json")
FORBIDDEN_CONTENT = (
    b"C:" + b"\\Users\\",
    b"C:" + b"\\Code\\",
    b"/home/" + b"runner/",
...
def residue_errors(archive: Archive, sdist_root: str) -> list[str]:
    ...
    for name, payload in archive.files.items():
        ...
        if (
            ...
            or (filename.startswith("page_") and filename.endswith(".jpg"))
        ):
...
def wheel_errors(archive: Archive, repository: Path, name: str, version: str) -> list[str]:
    errors: list[str] = []
    dist_info = f"{normalized_name(name)}-{version}.dist-info"
    expected = {
    ...
    expected.update(source_files(repository, PACKAGE))
    missing = sorted(expected - archive.files.keys())
    extra = sorted(archive.files.keys() - expected)
    errors.extend(f"{archive.path.name} is missing required wheel member: {item}" for item in missing)
    errors.extend(f"{archive.path.name} contains unexpected wheel member: {item}" for item in extra)
...
def sdist_errors(archive: Archive, repository: Path, name: str, version: str) -> list[str]:
    errors: list[str] = []
    root = f"{normalized_name(name)}-{version}"
    required = {
        "LICENSE",
        "MANIFEST.in",
        "README.md",
...
def inspect(directory: Path, repository: Path) -> tuple[list[str], tuple[Archive, ...]]:
    ...
    sdist_root = f"{normalized_name(name)}-{version}"
    errors: list[str] = []
